...
class DocumentHandler {
  ...
  deleteProperty(target, name) {
    // We need to save a state with the original value before deleting so
    // it may be restored in the future. If a state already exists, we need
    // to modify to keep the original value.
    if (this.states.hasOwnProperty(name)) {
      this.states[name].name = PropertyState.DELETED;
    } else {
      this.states[name] = new PropertyState(PropertyState.DELETED, target[name]);
    }
